...
const Stats = () => {
  ...
  if (error) {
    return (
      <Alert variant="danger" className="mt-4 text-center mx-auto shadow-sm" style={{ maxWidth: "600px" }}>
        <Alert.Heading className="fw-bold">🚨 TRUY CẬP BỊ TỪ CHỐI</Alert.Heading>
        <p className="my-3">{error}</p>
        <hr />
        <p className="mb-0 small text-muted">Vui lòng kiểm tra quyền tài khoản hoặc đăng nhập lại.</p>
      </Alert>
    );
  }

  const years = [...new Set(revenue.map(item => item[1]))].sort((a, b) => b - a);
  const filteredRevenue = revenue.filter(item => item[1].toString() === selectedYear);
        ...
        callbacks: {
          label: function (context) {
            ...
            if (context.parsed.y !== null) {
              label += Number(context.parsed.y).toLocaleString("vi-VN") + " VND";
            }
            return label;
          },
        },
  ...
  return (
    <>
      <Row className="mb-4 mt-2 align-items-center">
        <Col md={8}>
          <h2 className="text-uppercase fw-bold text-dark">Báo Cáo Thống Kê Doanh Thu</h2>
          <p className="text-muted small">Tổng hợp số tiền từ các hóa đơn theo từng tháng</p>
        </Col>
        <Col md={4}>
          <Form.Group>
            <Form.Label className="fw-bold text-secondary">Chọn năm thống kê:</Form.Label>
        ...
        <Row>
          <Col lg={8} md={12} className="p-2">
            <Card className="shadow-sm border-2 h-100">
              <Card.Body>
                <Card.Title className="fw-bold text-secondary mb-4">Biểu đồ doanh thu tài chính năm {selectedYear}</Card.Title>
          ...
          <Col lg={4} md={12} className="p-2">
            <Card className="shadow-sm border-2 h-100">
              <Card.Body className="p-0">
                <div className="p-3">
                  <Card.Title className="fw-bold text-secondary mb-0">Chi tiết số liệu năm {selectedYear}</Card.Title>
                </div>
