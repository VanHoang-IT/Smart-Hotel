...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    if (vnpResponseCode !== null && !hasProcessedPayment.current) {
      ...
      if (vnpResponseCode === "00") {
        ...
        alert("Giao dịch VNPay thất bại. Vui lòng thử lại!");
  ...
  const createCustomerProfile = async () => {
    ...
    try {
      ...
      alert(ex.response?.data?.message || "Khong tao duoc customer profile.");
    } finally {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        if (methodType === "CASH") {
          ...
          alert(
            "Đặt trước thành công, vui lòng đem tiền vào ngày checkIn để thanh toán",
          );
          finishPayment();
      ...
      let errorMsg =
        ex.response && ex.response.data ? ex.response.data : ex.message;
  ...
  return (
    <Container className="mt-5 mb-5">
      <h2 className="text-center mb-4 text-primary fw-bold">
        Danh sách phòng cần thanh toán
      </h2>
      <Row>
        <Col lg={8}>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body className="text-center p-4">
          <p className="text-muted mb-4">
            Vui lòng chọn một hình thức thanh toán thuận tiện nhất cho bạn.
          </p>
